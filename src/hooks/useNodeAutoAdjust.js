// useNodeAutoAdjust.js
// Node auto-adjustment hook to prevent overlapping - matching starter kit
...
function getBestMove(overlapNode, allNodes) {
  return overlapNode.overlap.directions.reduce((best, direction) => {
  ...
  return useCallback(
    (node) => {
      const currentNodeCoordinates = {
        ...
        x: node.position.x,
        y: node.position.y,
        w: node.measured?.width || node.width || 256,
        h: node.measured?.height || node.height || 100,
      };
      ...
      const nodes = getNodes().reduce((acc, n) => {
        if (n.id !== currentNodeCoordinates.id) {
          acc.push({
            ...
            x: n.position.x,
            y: n.position.y,
            w: n.measured?.width || n.width || 256,
      ...
      adjustedNodes.forEach(({ id, x, y }) => {
        ...
        const startX = _node?.position.x || 0;
        const startY = _node?.position.y || 0;
